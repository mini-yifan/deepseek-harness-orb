...
Host 就绪后才会创建球，出现在主显示器右沿、垂直方向中间稍下。悬停展开与主窗口外观一致的面板，球留在输入胶囊一角。单击固定面板；再点取消固定，指针离开后折叠。拖动移动球；大约五分之一越出左或右屏幕边缘后松开，球滑出变成灰色细条，稍等再悬停会滑回整颗球。面板显示 Compact 对话（与主窗口相同的对话记录）、绕球折行并随内容长高的输入（回车发送，Shift+Enter 换行）、仅 Computer Use 会话运行时出现在输入胶囊里、与球相对一端的停止，以及左上角**历史**以列出并重新打开 `dsh_orb` 上的 Computer Use 对话，历史和新建之间的 **Access** 芯片（仅可查看、工作区内修改或完全权限；完全权限无需确认），右上角**新建**以在该文件夹上再开一条 Computer Use 对话。当 Computer Use agent 向用户提问时，展开面板会显示题目和选项或输入框，可直接在球上作答；Compact 对话留在输入框上方，主窗口仍可回答同一请求。没有发送按钮、模式选择器、语音或圈选。
...
- 短查询（今天天气、当前新闻标题）用球上对话里的 web_search 或 web_fetch。Agent 判断再搜一次就能落到要点时，第二次搜索也留在这里。它不会启动后台 Agent，也不会在 GUI 里点来点去。
- 一段文件查找，或用户要的文件（Word、PPT、Excel、网站，或写成 HTML 的调研报告）会启动后台 Code agent。球会告诉你它正在运行。不依赖该结果的点击仍可在这一轮接着做，然后结束这一轮。必须等结果的点击会等到完成通知。
- 需要截图文件时用 screenshot：把捕获写到桌面并复制到剪贴板。
- 同一件后台产物的后续修改（刚写完 Word，再说把字体改成绿色），或同一段查找的下一段，会在已有标准会话上再发一条消息，即使它还在运行。
...
入队之后，Computer Use agent 会告诉你后台会话正在运行。不依赖该结果的点击可以在这一轮继续，否则结束这一轮，因此你可以继续聊天。那条标准会话结束且球空闲后，Computer Use 再决定下一段：剩下的点击、同一会话上的又一段后台工作，然后用几句话收束。入队的任务要求后台 agent 用几句话回答后停下，除非任务本身是要一份文件。

这些后台会话和你在主窗口里打字新建的会话是同一种。文件与终端由那条标准 Agent 使用 bash/fs。球自己的 bash 用于能回答你、或供下一步点击使用的命令。一段文件查找交给后台 agent。
