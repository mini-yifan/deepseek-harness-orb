...
`dsh-app://shell/` 无需联系 Host 即可提供打包的更新文档、脚本和样式。静态请求保留 GET/HEAD、路径范围和 MIME 处理；每个更新文档继续使用隔离 preload 和所属窗口的 IPC 校验。悬浮球 shell 页也由该来源提供；其 `dsh-app://app/index.html?surface=overlay` 的 transcript（文本记录）iframe 只在 Host 就绪时创建，并在每一代 Host 上重建。
...
- 主窗口仍可被截取。悬浮球、选择工具栏和观察框彩带可见时，macOS 在 DeepSeek Orb 进程内用 ScreenCaptureKit 窗口排除把它们从 Computer Use 截图中去掉。HID 点击穿透作用于悬浮球，并在该段操作期间隐藏工具栏。见 [Computer Use 观察框彩带](../../.agents/notes/implemented/feature/2026-09-19-computer-use-observation-frame.zh.md)。
