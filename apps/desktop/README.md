...
- After a macOS drag-select, Search / Translate / Agent Explain appear on a no-activate toolbar. Translate and Explain prompt the ball’s Computer Use session; Search opens Bing. Windows has no toolbar.
- Computer Use ships as a signed runtime extra under `extraResources/dsh`, not as a Desktop Host npm dependency. The main window stays capturable. The macOS overlay and selection toolbar are omitted from Computer Use screenshots by ScreenCaptureKit window exclusion; HID click-through applies to the ball and hides the toolbar for that burst.
