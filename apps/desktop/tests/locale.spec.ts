...
describe('desktop locale dictionaries', () => {
  ...
  it('owns overlay copy for placeholder, stop, new conversation, history, and disconnect', () => {
    ...
    expect(en.tccTitle).toBe('Desktop agent needs two Mac permissions')
    expect(zh.tccTitle).toBe('使用桌面 agent 需要两项 Mac 权限')
    expect(en.floatingQuit).toBe('Quit DeepSeek Orb')
    expect(zh.floatingQuit).toBe('退出 DeepSeek Orb')
  ...
  it('keeps visible plugin-manager and floating HTML copy in the locale dictionaries', () => {
    ...
    const frame = readFileSync(new URL('../renderer/observation-frame.html', import.meta.url), 'utf8')
    const staticText = [...`${plugin}\n${floating}\n${toolbar}\n${frame}`.matchAll(/>([^<]*\p{L}[^<]*)</gu)].map(match => match[1]?.trim())
