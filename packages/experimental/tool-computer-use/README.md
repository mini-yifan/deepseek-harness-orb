...
On macOS, grant Screen Recording to capture and Accessibility to post clicks and keys. Missing permission fails the capture or input with a message that names that TCC right. Windows and Linux still load; every backend method then throws `computer-use: desktop control is implemented only on macOS`.
