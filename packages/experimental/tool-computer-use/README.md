...
On macOS, grant Screen Recording to capture, Accessibility to post clicks and keys, and Automation for Finder. Missing Screen Recording or Accessibility fails the capture or input with a message that names that TCC right. Windows captures with GDI and posts input with `SendInput`, both in per-monitor physical pixels, so a position on the screenshot lands on that pixel on every display in a mixed-DPI layout. An elevated target window rejects clicks and typing. Linux still loads, and every backend method then throws `computer-use: desktop control is implemented only on macOS and Windows`.
