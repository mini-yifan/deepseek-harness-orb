...
On macOS, grant Screen Recording to capture, Accessibility to post clicks and keys, and Automation for Finder. Desktop's overlay covers Screen Recording and Accessibility before overlay send; the plugin still fails capture or input with a message that names that TCC right if a right is missing at execute. Finder Automation prompts on first use. Windows captures with GDI and posts input with `SendInput`; an elevated target window rejects clicks and typing. Linux still loads, and every backend method then throws `computer-use: desktop control is implemented only on macOS and Windows`.
