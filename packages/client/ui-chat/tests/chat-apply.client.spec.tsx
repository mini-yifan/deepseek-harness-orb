...
describe('Chat apply wiring', () => {
  ...
  it('forces Compact transcript on the overlay surface even when Host Settings are Normal', async () => {
    ...
    try {
      ...
      expect(face.hooks.transcriptView.getSnapshot()).toBe('compact')
      b.chatSettings.publish({
        status: 'ready', value: { transcriptView: 'normal' }, revision: 1, writable: true,
      })
      expect(face.hooks.transcriptView.getSnapshot()).toBe('compact')
      face.setTranscriptView('normal')
      expect(face.hooks.transcriptView.getSnapshot()).toBe('compact')
