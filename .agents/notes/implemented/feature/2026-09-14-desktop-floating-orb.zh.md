...
overlay 是 72px 球：悬停以 300ms 展开跟随 Host 外观的 320×420 面板，球留在输入胶囊一角。单击固定面板；再点取消固定，指针离开两者后 180ms 折叠。指针仍在 overlay 上时取消固定会保持面板展开。收起时球留在展开角，直到窗口缩到带留白的收起尺寸。[悬浮球 CSS 投影与固定描边](2026-09-17-floating-orb-chrome-shadow.zh.md) 拥有 12px 窗口留白、投影、细边和外圈固定描边。主键拖动按球上的指针偏移记录，立刻收起，再按球原点移动，已存储的展开方向不会带动球；[悬浮球只用主键拖动](../bug-fix/2026-09-17-floating-orb-primary-pointer-drag.zh.md) 拥有哪些按钮启动这次抓取。pointerup 时把自由浮球夹入工作区；左右吸边见[悬浮球左右吸边](2026-09-20-floating-orb-edge-dock.zh.md)。输入框绕球折行并朝对话记录长高；回车发送，Shift+Enter 换行（[overlay 输入框绕球长高](2026-09-19-overlay-growing-composer.zh.md)）。停止是 body 兄弟（`#ball` z-index 1，`#stop` z-index 2），放在输入胶囊里与球相对的一端，仅 Computer Use 会话运行时显示并替换被隐藏的输入；点击对该 overlay 会话调用 `session/cancel`。历史会把 Compact 对话记录 iframe 换成 cwd 为 `dsh_orb` 且投影 preset 为 `computer-use` 的会话；委派出去的 standard 与 `code_agent` 行不进该列表。历史和新建放在输入胶囊对侧的对话记录边上；[Overlay 历史与新建跟随输入胶囊对侧](../bug-fix/2026-09-17-overlay-history-new-follow-expand.zh.md) 拥有该位置。Access 芯片放在两者之间；[overlay Access 选择器](2026-09-17-overlay-access-picker.zh.md) 拥有该控件。点击一行会把该 id 写入 `floating-session.json` 并 post 进 iframe，后续提示词继续那条对话。新建会在 `dsh_orb` 上再开一条 Computer Use 会话。当这条 Computer Use 会话发起 `ask_user_question` 时，overlay 认领仍在进行的 `'user-questions/request'` waterfall，并在展开面板里渲染紧凑答题卡；[悬浮球回答用户提问](2026-09-15-floating-orb-user-questions.zh.md) 拥有这条 Client 路径。

`code_agent` 只注册在 Computer Use preset。创建走 `session.create({ agentPreset: 'standard' })`，不设 `origin: 'subagent'`，也不传 `parentAgent`。Desktop 新建会在 prompt 之前应用可选的 `orbCodeAgentModel`；续写不会。发送任务走 `mode: 'queue'`，返回 `{ accepted: true }`，不等待回合结束。Computer Use 拥有的监视之后会把插件通知停到两边都空闲再投递；[Computer Use 把 Code agent 完成通知停到空闲再投递](2026-09-15-computer-use-code-agent-completion.zh.md) 拥有这条投递路径。省略 `session_id` 会新建空白 standard 会话；带上先前结果 id 则在那条会话再入队一条用户消息。[Overlay Computer Use 后台调度](2026-09-17-orb-code-agent-dispatch.zh.md) 拥有新建 cwd、调用方登记表、`code_agent_status` / `code_agent_stop`，以及 Code agent 上的无人值守应答。Computer Use 策略要求模型把可见 GUI 留在 GUI 工具上，把长任务交给 `code_agent`，把短查询留在网页工具上，用先前 id 续写同一产物，无关新工作则不带 id，入队后告诉用户后台 agent 正在运行并结束回合，并汇报之后的插件通知。
