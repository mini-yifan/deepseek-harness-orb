...
The overlay is one 72px GIF ball that hover-expands a 320×420 white panel (300ms) while the ball stays in the input-pill corner. Click pins the panel; click again unpins, and the pointer leaving both then collapses after 180ms. Drag moves the ball and clamps it inside the work area; there is no edge dock. Enter sends; Stop shows only while the Computer Use session is running; a New control mints another Computer Use session on `dsh_orb`.
