...
The authored headless overlay [`snapshots/session/computer-use/`](../../../../snapshots/session/computer-use/) mounts a scenario-local fake-desktop plugin plus vision model `deepseek-v4-flash-vision-exp` with `postActionWaitMs: 0`. Replay uses a fixture PNG and never drives a real desktop. The overlay stubs `sessionController` and registers `code_agent` so the header pin includes that schema; headless has no Session Remote. The plugin is not in shipped `dsh-base`.
