...
`observeDesktop` calls `DesktopBackend.inspectForeground` once, then captures screens. The model-facing content is one foreground text block followed by the existing per-screen envelopes and images. Overlay skip uses `activeCaptureExcludeWindowIds()` (the ball and expanded panel share that window). The inspect path does not skip the Electron PID. If the Desktop main window is next in z-order, `<frontmost_app>` is that window's localized name (typically DeepSeek Harness) and fallback is not used.
...
**Window title, Office document path, or other apps' folders.** Phase 1 is app name, Finder folder, and focus fallback.

## Consequences

Every observation spends a few tokens on OS metadata. Finder folder lookup needs Automation for Finder; a missing grant omits the folder and keeps the app name. Overlay-only skip can name the Desktop main window as `<frontmost_app>`. Inspect failures still attach screens, with fallback tags.
