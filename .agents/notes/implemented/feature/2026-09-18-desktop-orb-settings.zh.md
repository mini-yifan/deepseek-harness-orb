...
自定义图片拷进配置目录的 `orb-avatar` 与 `orb-avatar.json`（GIF/PNG/WebP，2 MB 上限）。`dsh-app://app/orb-avatar` 与 `dsh-app://shell/orb-avatar` 提供该文件或 `deepseek-avatar-square.gif`。叠加与后台模型仍写入 `orb-agent-models.json`；划词开关调用 `SelectionToolbarController.setEnabled`。Access、打开主窗口和退出留在原生菜单。目录读取仍走 Host `session/modelCatalog`。
