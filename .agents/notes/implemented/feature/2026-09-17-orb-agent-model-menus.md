...
The overlay native context menu, rebuilt on every right-click from live `session/modelCatalog`, adds Floating Agent Settings and Background Agent Settings after Open Main. Each submenu groups by catalog `groups[]` (`group.name` as a disabled header, persist `group.id`). A model with `reasoning.efforts` is a submenu of consecutive `radio` items using `effort.name`; the current model prefixes that row with ✓ because Electron check items and submenus are exclusive, so assigning `submenu` after a checkbox throws and a native check cannot appear on a thinking-mode parent. A model with no reasoning is a leaf `checkbox`. Never attach `submenu: []`. Effective effort is `current.reasoningEffort ?? defaultEffort`. A Default radio is added only when efforts exist and `defaultEffort` is absent. Overlay lists every catalog model. Checkmarks are independent between the two agents. Clicking an effort selects that provider + model + reasoningEffort.

Selections persist as Desktop profile `orb-agent-models.json` (`{ overlay, background }`), not inside `floating-session.json`. Absent or invalid JSON uses `{ provider: 'deepseek-official', model: 'deepseek-flash', reasoningEffort: 'max' }` for both. The main-window [Desktop orb Settings](2026-09-18-desktop-orb-settings.md) page writes the same JSON immediately.
