...
`long_wait` requires `wait_seconds` in `{20, 30, 60, 120}`. Missing or off-enum values fail loud. Use it only for a visible long job (download, installer, export, in-window generation). Pick the smallest bucket that covers remaining progress; 120 only when the screenshot already shows a minutes-long job. The 20-second floor is the product invariant that keeps ordinary refresh on `wait`. Both tools are exclusive GUI recapture; `delay()` still honors abort.
...
**A 5–15 second middle tool.** That reopens short “insurance” pauses. The gap is filled by repeating `wait` or paying the 20-second `long_wait` cost.
...
The Computer Use catalog is ten exclusive GUI tools plus `code_agent`. Ordinary loaders cannot request 3–5 seconds. A remaining 10-second download either loops `wait` or overshoots with `long_wait` 20. Overlay-guard is unchanged: neither tool posts HID.
