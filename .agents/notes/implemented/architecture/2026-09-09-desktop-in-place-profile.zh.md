...
Desktop 在插件管理窗口应用变更时停止 Host，然后直接修改当前 profile。插件市场在 Host 仍存活时修改同一 profile；用户重启后加载新的 Loader 条目。修改包前解除宿主共享链接，操作结束后恢复链接。保留包锁、依赖验证和已批准的原生构建。兼容升级只刷新链接，不复制插件文件。
