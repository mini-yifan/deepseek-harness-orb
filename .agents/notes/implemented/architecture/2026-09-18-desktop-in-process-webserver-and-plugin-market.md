...
Before Loader entries mount, Desktop Host provides `desktopProfiles.current = { name: 'desktop', dir }` and `desktopPnpm` with `runPlugin` / `runExternalMarketPluginInstall`. Those methods send Host→Electron `plugin-run` IPC. Electron runs bundled pnpm through `DesktopProjectManager.mutateWhileRunning` without stopping the Host, then the market UI tells the user to restart. GitHub, gist, git, file, and URL specs remain rejected.

Unpackaged launches persist external plugins in `$DSH_HOME/profiles/desktop` and relink them into the wiped hoist project. The linker then places each plugin's host peer packages from the hoist beside the store copies, because Node ESM realpath of a store plugin does not search the hoist `node_modules`. `start:desktop` pins `dshmarket@1.50.0` in that store. `--skip-build` rebuilds `@deepseek-ai/dsh-host-webserver` and `@deepseek-ai/dsh-client-modules` so Host loads `dispatch` and the `/plugins` carrier registration from those packages' `lib/index.js`. Client-modules registers that route through `ctx.get('webServer')` when the carrier is already provided, because Cordis property access requires inject on a plugin fiber. The market package is not a signed `DESKTOP_PROFILE_BUNDLES` member. Packaged applications carry `dshmarket@1.50.0` as `extraResources/plugins/dshmarket-1.50.0.tgz` and seed the Desktop profile from that tarball when the exact spec is absent; first seed may still fetch the package's npm dependencies from the registry. Peer `satisfies` includes prereleases so a host `0.1.5-rc.2` matches caret-rc ranges. Packaged `applyRelease` disables every third-party bundle and continues when graph validation still fails.
...
Settings → Plugins can show Plugin Market after `start:desktop` once `dshmarket` is in the development store, and after a packaged launch once the bundled tarball has been seeded. Market installs of registry packages survive the next hoist rebuild. GitHub-only catalog entries fail with an explicit unsupported-spec error. Desktop still does not listen on TCP, still does not enable `web-runtime` or `client-hmr`, and still does not let the CLI manage `profile desktop`.

Tests cover `listen: false` with no TCPWRAP, `dispatch` prefix hits and misses, Host/Origin `sameOrigin` values, fetch order, development-store linking of `dshmarket` and its host peers, `github:` rejection, packaged-profile seed of the bundled tarball when the spec is absent versus skip when `dshmarket@1.50.0` is already recorded, and packaged `applyRelease` disabling third-party plugins when the graph cannot stay enabled. GUI browse/install against awesome-dsh-plugin remains a local Desktop run.
