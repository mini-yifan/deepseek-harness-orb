...
插件管理窗口在包修改前停止 Host，并等待 pnpm 退出后再重启它。插件市场在 Host 仍存活时修改 profile（[进程内 webServer 决策](2026-09-18-desktop-in-process-webserver-and-plugin-market.zh.md)）。[直接修改决策](2026-09-09-desktop-in-place-profile.zh.md)规定部分失败和持久重试状态的处理方式。记录的宿主链接用于识别自有目录，与包操作是否完成相互独立。
