...
The test vectors are re-pinned from the published calculator. The request-pricing tests, package README, and this note carry the token numbers. Catalog model ids are unchanged. The harness request-image pixel budget is owned by [the 1300×1300 projection note](2026-09-15-deepseek-request-image-pixel-budget.md).
...
**Raise the harness pixel budget in the same change.** Pricing the bytes actually sent is independent of which raster the harness encodes. The request-content budget is [the 1300×1300 projection](2026-09-15-deepseek-request-image-pixel-budget.md).

## Consequences

An 800×800 request image costs 422 tokens instead of 349, while a 640×480 image costs 206 instead of 209 and a low-budget 512×512 image costs 184 instead of 201. Compaction pressure changes with the retained image dimensions. A pixel budget does not imply a 422-token ceiling: an 8192×1 image stays within the omitted-model budget and costs 1024 tokens. The estimate no longer carries a three-token conservative margin; provider usage remains the authoritative anchor once a request completes. Sessions replayed through `llm-replay` use their fixture's `imageRequestTokens` and are unaffected.
