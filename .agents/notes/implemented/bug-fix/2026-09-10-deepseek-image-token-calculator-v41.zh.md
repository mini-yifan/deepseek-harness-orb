...
测试向量按公开计算器重新固定。request-pricing 测试、包 README 和本 note 使用 token 数字。catalog 模型 id 不变。harness 请求图片像素预算由[1300×1300 投影记录](2026-09-15-deepseek-request-image-pixel-budget.zh.md)拥有。
...
**在同一改动中提高 harness 像素预算。** 为实际发送的字节计价，与 harness 编码哪一张光栅无关。请求内容预算是[1300×1300 投影](2026-09-15-deepseek-request-image-pixel-budget.zh.md)。

## 后果

800×800 请求图片的计价从 349 变为 422 token，640×480 图片从 209 变为 206，低预算下的 512×512 图片从 201 变为 184。压缩压力随保留图片的尺寸变化。像素预算不意味着 422 token 上限：8192×1 图片仍在省略模型预算内，并计 1024 token。估算值不再带 3 token 的保守余量；请求完成后，提供方 usage 仍是权威锚点。经 `llm-replay` 回放的会话使用各自 fixture 的 `imageRequestTokens`，不受影响。
