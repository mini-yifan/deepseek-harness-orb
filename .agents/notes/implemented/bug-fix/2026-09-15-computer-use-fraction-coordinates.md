...
POLICY, the first-frame notice, and the `position` parameter descriptions state that `[0, 0]` is the top-left of the visible screenshot, `[1000, 1000]` is the bottom-right, x and y scale independently, and pixel widths, request-preview sizes, and other image-handle dimensions must be ignored. `mapNormalizedToGlobal` is unchanged.
...
**Strip `request preview WxHpx` from the global image handle.** [`requestImageHandleText`](../../../../packages/llm/llm/src/content.ts) is shared by every vision route. `read_image` still names original-file multipliers so the model can locate features on disk.

## Consequences

The adapter still prepends request-preview dimensions; POLICY tells the model to ignore them. Retina capture, 2048×2048 attachment normalization, and the 1,690,000-pixel request budget still resize rasters; they no longer appear as a second coordinate space on the Computer Use observation. HID, overlay-guard, and ScreenCaptureKit capture are unchanged.
