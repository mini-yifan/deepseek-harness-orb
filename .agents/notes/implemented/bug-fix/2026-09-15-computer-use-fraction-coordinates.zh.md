...
Computer Use 把 `click` / `input_text` / `scroll` 的 `position` 按每屏逻辑边界上相互独立的 0–1000 比例映射，再发送该全局点。面向模型的信封和策略却标出附件像素、逻辑尺寸，以及指向原始捕获的缩放倍率，并让模型在选坐标前用这些倍率转换附件像素。DeepSeek 请求序列化仍会在每张图片旁写上 `request preview WxHpx`。视觉模型只要跟着其中任一像素尺寸走，就会在截图已经清楚时打偏明显按钮。
...
POLICY、首帧通知和 `position` 参数描述写明：`[0, 0]` 是可见截图左上角，`[1000, 1000]` 是右下角，x 与 y 独立缩放，并且必须忽略像素宽度、请求预览尺寸和其他图片句柄尺寸。`mapNormalizedToGlobal` 不变。
...
**从全局图片句柄去掉 `request preview WxHpx`。** [`requestImageHandleText`](../../../../packages/llm/llm/src/content.ts) 由每条视觉路由共用。`read_image` 仍会标出原文件倍率，以便模型在磁盘上定位特征。

## 影响

适配器仍会前置请求预览尺寸；POLICY 告诉模型忽略它们。Retina 捕获、2048×2048 附件规范化，以及 1,690,000 像素请求预算仍会缩放栅格；它们不再作为第二套坐标系出现在 Computer Use 观察上。HID、overlay-guard 和 ScreenCaptureKit 捕获不变。
